...
preservePermissions:(BOOL)preservepermissions
{
	...
	if(creation)
	{
		...
		*((struct timespec *)attrptr) = [XADDate timespecStructFromDate:creation];
		attrptr+=sizeof(struct timeval);
	}
	if(modification)
	{
		list.commonattr|=ATTR_CMN_MODTIME;
		*((struct timespec *)attrptr) = [XADDate timespecStructFromDate:modification];
		attrptr+=sizeof(struct timeval);
	}
	if(access)
	{
		list.commonattr|=ATTR_CMN_ACCTIME;
		*((struct timespec *)attrptr) = [XADDate timespecStructFromDate:access];
		attrptr+=sizeof(struct timeval);
	}
