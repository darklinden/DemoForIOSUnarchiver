		...
		switch(entryid)
		{
			...
			case 8: // Dates
			{
				...
				if(entrylen>=4)
				{
					...
					[shared setObject:[XADDate XADDateWithTimeIntervalSince2000:creation] forKey:XADCreationDateKey];
				}

				if(entrylen>=8)
				{
					uint32_t modification=[fh readUInt32BE];
					[shared setObject:[XADDate XADDateWithTimeIntervalSince2000:modification] forKey:XADLastModificationDateKey];
				}

				if(entrylen>=12)
				{
					uint32_t backup=[fh readUInt32BE];
					[shared setObject:[XADDate XADDateWithTimeIntervalSince2000:backup] forKey:XADLastBackupDateKey];
				}

				if(entrylen>=16)
				{
					uint32_t access=[fh readUInt32BE];
					[shared setObject:[XADDate XADDateWithTimeIntervalSince2000:access] forKey:XADLastAccessDateKey];
				}
			}
			break;
