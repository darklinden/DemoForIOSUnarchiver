	...
	while(length>9)
	{
		...
		else if(extid==0x334d&&size>=14) // Info-ZIP Macintosh Extra Field
		{
			...
			if(mh&&len>=26)
			{
				...
				if(create>=86400) [dict setObject:[XADDate XADDateWithTimeIntervalSince1904:create] forKey:XADCreationDateKey];
				if(modify>=86400) [dict setObject:[XADDate XADDateWithTimeIntervalSince1904:modify] forKey:XADLastModificationDateKey];
				if(backup>=86400) [dict setObject:[XADDate XADDateWithTimeIntervalSince1904:backup] forKey:@"MacOSBackupDate"];
			}
		}
...
isLastEntry:(BOOL)islastentry
{
	NSMutableDictionary *dict=[NSMutableDictionary dictionaryWithObjectsAndKeys:
		...
		[XADDate XADDateWithMSDOSDateTime:date],XADLastModificationDateKey,
