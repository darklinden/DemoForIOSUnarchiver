...
{
	return [self XADDateWithTimeIntervalSince1601:(double)filetime/10000000];
}

+(NSDate *)XADDateWithWindowsFileTimeLow:(uint32_t)low high:(uint32_t)high
{
	return [self XADDateWithWindowsFileTime:((uint64_t)high<<32)|(uint64_t)low];
}
...
#ifndef __MINGW32__
+(struct timeval)timevalStructFromDate:(NSDate *)date
{
...
+(struct timespec)timespecStructFromDate:(NSDate *)date
{
	NSTimeInterval seconds=[date timeIntervalSince1970];
