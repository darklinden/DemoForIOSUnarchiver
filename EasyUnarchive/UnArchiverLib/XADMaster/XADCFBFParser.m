	...
	while(dirsec!=0xfffffffe)
	{
		...
		for(int i=0;i<secsize;i+=128)
		{
			...
			else
			{
				...
				if(created) [entry setObject:[XADDate XADDateWithWindowsFileTime:created] forKey:XADCreationDateKey];
				if(modified) [entry setObject:[XADDate XADDateWithWindowsFileTime:modified] forKey:XADLastModificationDateKey];
