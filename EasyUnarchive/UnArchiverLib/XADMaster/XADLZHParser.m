...
-(void)parseWithSeparateMacForks
{
	...
	while([self shouldKeepParsing] && ![fh atEndOfFile])
	{
		...
		if(level==0||level==1)
		{
			...
			[dict setObject:[XADDate XADDateWithMSDOSDateTime:time] forKey:XADLastModificationDateKey];
...
-(void)parseExtendedForDictionary:(NSMutableDictionary *)dict size:(int)size
{
	...
	switch([fh readUInt8])
	{
		...
		case 0x41:
			[dict setObject:[XADDate XADDateWithWindowsFileTimeLow:[fh readUInt32LE]
