...
-(void *)xadFileInfoForEntry:(int)n
{
	...
	if(mod)
	{
        ...
        NSDateComponents *dayComponents = [calendar components:(NSYearCalendarUnit) fromDate:mod];
        
		fi->xfi_Date.xd_Year=[dayComponents year];
		fi->xfi_Date.xd_Month=[dayComponents month];
		fi->xfi_Date.xd_Day=[dayComponents day];;
		fi->xfi_Date.xd_Hour=[dayComponents hour];
		fi->xfi_Date.xd_Minute=[dayComponents minute];
		fi->xfi_Date.xd_Second=[dayComponents second];
	}
